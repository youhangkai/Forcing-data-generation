...
zero = 0;
inputfile_dir = "H:\radiation\MERRA2_Rad_1980to2000\";
...
for i = Start_year:End_year
    ...
    parfor j = ST:12
        ...
        for k = 1:month_day
            ...
            for hr = 2:3:23
                hour = ceil(hr/3);
                ori_surface_emissivity_map = ncread(input_file_path,'EMIS');
                ori_surface_absorbed_longwave_radiation_map = ncread(input_file_path,'LWGAB');
                ori_longwave_radiation_map = ori_surface_absorbed_longwave_radiation_map ./ ori_surface_emissivity_map;
                temp_longwave_radiation_map(:,:,(8*(k-1))+hour) = fliplr(ori_longwave_radiation_map(:,:,hr));
            end
        ...
        % for incident longwave radiation
        ncid_new = netcdf.open(output_file_path,'WRITE');
        read_CFT_id = netcdf.inqVarID(ncid_new,'FLDS');
